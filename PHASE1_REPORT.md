```markdown
<div dir="rtl" align="justify" markdown="1">

# گزارش فاز اول: طراحی سامانه هوشمند ارزیابی اعتبار GNSS و ثبت داده‌های مکانی و حرکتی

---

## ۱. مقدمه و هدف فاز اول
هدف از اجرای فاز اول پروژه، طراحی و پیاده‌سازی زیرساخت اصلی اپلیکیشن اندروید جهت دریافت همزمان، همگام‌سازی، اعتبارسنجی و ذخیره‌سازی داده‌های مکانی (GNSS) و حسگرهای حرکتی (IMU) است. در این فاز، سامانه به‌گونه‌ای پیاده‌سازی شده است که بتواند بدون وابستگی به سخت‌افزار خاص یا دسترسی روت (Root)، بر روی تمامی دستگاه‌های استاندارد اندرویدی اجرا شده و بستری پایدار برای الگوریتم‌های تخمین موقعیت در فازهای بعدی فراهم سازد.

---

## ۲. معماری نرم‌افزار و جریان داده (Architecture & Data Flow)&rlm;
معماری نرم‌افزار بر پایه الگوی **MVVM (Model-View-ViewModel)** و اصول **Clean Architecture** به همراه کتابخانه رابط کاربری مدرن **Jetpack Compose** پیاده‌سازی شده است.

ساختار کلی جریان داده‌ها در نرم‌افزار به صورت زیر است:
...
۳. **ژیروسکوپ (Gyroscope):** سنجش سرعت زاویه‌ای دوران دستگاه در سه محور بر حسب رادیان بر ثانیه ($\text{rad/s}$).
...
| ۲ | دریافت مجوزهای لازم (Location, Sensors, Notification) | ✅ انجام شد | [`DashboardScreen.kt`](https://www.google.com/search?q=app/src/main/java/com/example/iotproject/ui/screens/DashboardScreen.kt) |
| ۳ | خواندن اطلاعات Location (طول، عرض، دقت، سرعت، جهت) | ✅ انجام شد | [`LocationDataManager.kt`](https://www.google.com/search?q=app/src/main/java/com/example/iotproject/data/location/LocationDataManager.kt) |
| ۴ | خواندن سنسورهای شتاب‌سنج، ژیروسکوپ، مغناطیس‌سنج، بردار چرخش و گام‌شمار | ✅ انجام شد | [`SensorDataManager.kt`](https://www.google.com/search?q=app/src/main/java/com/example/iotproject/data/sensor/SensorDataManager.kt) |
| ۵ | ثبت Timestampهای مناسب جهت همگام‌سازی زمانی | ✅ انجام شد | فیلدهای `timestamp_ms` و `elapsedRealtimeNanos` |
| ۶ | تولید و ذخیره داده‌ها در قالب فایل CSV | ✅ انجام شد | [`DataLogger.kt`](https://www.google.com/search?q=app/src/main/java/com/example/iotproject/data/logging/DataLogger.kt) |
| ۷ | تعریف ۴ حالت عملکردی Healthy, Degraded, Suspicious, Unavailable | ✅ انجام شد | [`GnssStatusState.kt`](https://www.google.com/search?q=app/src/main/java/com/example/iotproject/data/model/GnssStatusState.kt) |
| ۸ | طراحی و پیاده‌سازی ماژول ارزیابی اعتبار GNSS با مقایسه کینماتیک سنسورها | ✅ انجام شد | [`GnssIntegrityEvaluator.kt`](https://www.google.com/search?q=app/src/main/java/com/example/iotproject/domain/assessment/GnssIntegrityEvaluator.kt) |
